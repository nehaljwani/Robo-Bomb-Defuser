...
class Gun:
    ...
    def __init__(self,Arena,Y,X,Pos,LINES,COLS,Level):
        ...
        if LEVEL==2:
            ...
            list(map(lambda x: [Arena.addch(Y+y,X+x,'#') for y in range(3)],list(range(3))))
            if Pos==0:
                Arena.addch(Y,X+2,' ')
                Arena.addch(Y+2,X+2,' ')
            if Pos==1:
                ...
                Arena.addch(Y+2,X,' ')
        if LEVEL==3:
            if Pos ==0: list(map(lambda x: Arena.addstr(1,x,"### "),list(range(3,COLS-7,4))))
            else:  list(map(lambda x: Arena.addstr(LINES-1,x,"### "),list(range(3,COLS-7,4))))
            self.yx=list(range(4,COLS-3,4))
...
class MyGame(curses_window):
    ...
    def Start(self,Game,LINES,COLS,LEVEL):
        ...
        list(map(lambda x: Game.addch(x[0],x[1],'D'),DC_xy)) #Placing Diffuce Codes & Bomb
        list(map(lambda x: Game.addch(x[0],x[1],'0'),B_xy))
        global Score
        if LEVEL==1:
             Score=0
             Limit=401
        ...
        if LEVEL==3:
             ...
             La=[(1,x) for x in range(COLS)]
             mxa_loc,mya_loc=4,2
             Lb=[(LINES-1,x) for x in range(COLS)]
        ...
        while True:
            if Key==27:
                ...
                return -1,REASON
            if time==Limit:
                list(map(lambda x: [Game.addch(l+x,m+y,'0') for y in range(3)],list(range(3))))
            ...
            time+=1
            if flag==0:
                list(map(lambda x: [Game.addch(l+x,m+y,' ') for y in range(3)],list(range(3))))
                Game.addch(l+3//2,m+3//2,'0')
                flag=1
            else:
                list(map(lambda x: [Game.addch(l+x,m+y,'0') for y in range(3)],list(range(3))))
...
class Robot:
    ...
    def Place(self,y_loc,x_loc,scr):
        ...
        list(map(lambda i: [scr.addch(i,j,self.type[i-y_loc][j-x_loc]) for j in range(x_loc,x_loc+5)],list(range(y_loc,y_loc+4))))
    def Move(self,y_loc,x_loc,scr,Key):
        ...
        if Key==KEY_LEFT or Key==KEY_RIGHT:
            list(map(lambda i: scr.addch(i,x_loc+x_a+x_r,' '),list(range(start,end))))
        if Key==KEY_UP or Key==KEY_DOWN:
            list(map(lambda i: scr.addch(y_loc+y_a+y_r,i,' '),list(range(start,end))))
